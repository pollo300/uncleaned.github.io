...
const BUTT_1 = {
    ...
    build: { n: 'build', setup: t_canv02_setup },
    startmatch: { n: 'start', setup: t_canv0_setup },
...
function t_canv02_setup() {
    this.t = t_canv02
}
function t_canv02(A) {
    if (A._from_mousedown) {
        createPopup_CV02(this)
    }
    this.update()
}
...
function add_canvas_picker2(popup, ST = Pikerstats) {
    const stats = structuredClone(ST)
    ...
    stats.pressingHere = false;
    stats.cursorPosition = { x: stats.vx * stats.sx, y: stats.vy * stats.sy };
    stats.FUNCT = function () { console.log("fffffffssss") }
    console.log(stats.cursorPosition)
    stats.canvas = canvas
    stats.ctx = ctx
    // Ensure cursor stays within canvas bounds
    ...
    function draw() {
...

        {
            ...
            ctx.fillStyle = '#fff';
            ctx.lineWidth = 2;
            ctx.arc(stats.vx * canvas.width, stats.vy * canvas.height, 4, 0, Math.PI * 2);
            ctx.stroke();
            ctx.fill();
            ctx.closePath();
        }
    ...
    function handleMouseMove(e) {
        if (stats.pressingHere) {
            ...
            stats.cursorPosition = clampCursor(mouseX, mouseY);
            stats.vx = stats.cursorPosition.x / canvas.width
            stats.vy = stats.cursorPosition.y / canvas.height
            stats.update()
        }
    }
    {
...
function add_textdisplay(popup, ST = Pikerstats) {
    const stats = structuredClone(ST)
    ...
    stats.canvas = canvas
    stats.ctx = ctx
    stats.text = stats.text ?? ["text","missing"]
    // Ensure cursor stays within canvas bounds
    ...
    function draw() {
        ...
        ctx.beginPath();
        ctx.lineWidth = 6
        ctx.strokeStyle = "black";       // Border color for the circle
        ctx.fillStyle = "#FFFFFF"
        ctx.font = `bold ${Math.floor(24)}px Arial`;
        //ctx.textAlign = "center";
        //ctx.lineCap = 'round'

        ctx.strokeText(stats.text[0], canvas.width*0.06, canvas.height*0.8);
        ctx.fillText(stats.text[0], canvas.width*0.06, canvas.height*0.8);

        ctx.strokeText(stats.text[1], canvas.width*0.5, canvas.height*0.8);
        ctx.fillText(stats.text[1], canvas.width*0.5, canvas.height*0.8);
        //ctx.fillText("this.owner.display_name", canvas.width/2, canvas.height*0.8);
        ctx.closePath();
    }
...
function createPopup_CV02(BUTTON) {
    const popup = createPopup(BUTTON.n, 220, 380, 20)
    let ALL_UPDATER = []


    let stats = {
        colorY: ['rgb(95, 95, 95)', 'rgb(77, 91, 226)', 'rgb(0, 0, 0)'],
        colorX: ['rgb(85, 85, 85)', 'rgb(255, 255, 255)'],
        mode_c: "overlay",
        sx: 150,
        sy: 30,
        x: 0,
    ...
    function createstats(obj, key) {
        let piker = add_canvas_picker2(popup, stats)

        stats2.colorY[1] = stats.colorY[1]
        let text = add_textdisplay(popup, stats2)

        // initialize with the actual object’s property
        piker.vx = obj[key]

        piker.FUNCT = function () {
            piker.vy = 0.5
            // write directly into the real BU.fov
            obj[key] = piker.vx
        }

        text.FUNCT = function () {
            text.text[1] = (obj[key]*10).toFixed(1)
        }

        ALL_UPDATER.push(piker,text)
        stats.y += stats.sy
        stats2.y += stats2.sy
        return piker
    }

    stats.colorY[1] = 'rgb(77, 91, 226)'
    stats2.text[0] = "👁️‍🗨️"
    createstats(BU, "fov")
    stats.colorY[1] = 'rgb(121, 188, 206)'
        stats2.text[0] = "⏩"
    createstats(BU, "spead")
    stats.colorY[1] = 'rgb(135, 232, 152)'
        stats2.text[0] = "📈"
    createstats(BU, "accel")
    stats.colorY[1] = 'rgb(219, 162, 211)'
        stats2.text[0] = "💗"
    createstats(BU, "regen")
    stats.colorY[1] = 'rgb(203, 150, 131)'
        stats2.text[0] = "🩶"
    createstats(BU, "healt")
    stats.colorY[1] = 'rgb(157, 89, 89)'
        stats2.text[0] = "🪨"
    createstats(BU, "body_dmg")
    stats.colorY[1] = 'rgb(255, 98, 0)'
        stats2.text[0] = "☄️"
    createstats(BU, "bullet_hp")
    stats.colorY[1] = 'rgb(255, 0, 0)'
        stats2.text[0] = "💥"
    createstats(BU, "bullet_dmg")
    stats.colorY[1] = 'rgb(255, 166, 0)'
        stats2.text[0] = "💢"
    createstats(BU, "reload")
    stats.colorY[1] = 'rgb(164, 197, 243)'
        stats2.text[0] = "💨"
    createstats(BU, "b_shoot")
    stats.colorY[1] = 'rgb(148, 187, 197)'
        stats2.text[0] = "⚡"
    createstats(BU, "b_range")
    stats.colorY[1] = 'rgb(102, 127, 114)'
        stats2.text[0] = "⏰"
    createstats(BU, "b_time")
        stats.colorY[1] = 'rgba(255, 154, 154, 1)'
        stats2.text[0] = "🎯"
    createstats(BU, "precision")
        stats.colorY[1] = 'rgba(215, 143, 184, 1)'
        stats2.text[0] = "🌡️"
    createstats(BU, "b_spe_ran")
        stats.colorY[1] = 'rgba(255, 217, 66, 1)'
        stats2.text[0] = "🏛️"
    createstats(BU, "b_size")

    //💗 🩶 ​ ​ 💓 ​ ❤️‍🔥 ​ ❤️‍🩹 ​ 💢 ​ 🎯 🎯 💥 ​ 👁️‍🗨️ ​ ☄️ ​ ⚡ ​ 🗡️ ​ 🩸 ​ ⚙️ ​ 🔒 ​ 💦 ​ ⏰ ​ 🏌️ ​ 🏇 ​ 🚴‍♀️ 💎 💎 👨‍🦼‍➡️ ​ 🧑‍🧑‍🧒‍🧒 ​ 🕸️ ​ 🌬️ ​ 🫧 ​ 🔘​ 🔁 ​ ⏩ ​ 💣 ​ 🧿 ​ 💎​ 🛡️ 👁️​ 👀 ​ 🧭 ​ ✈️ 📈 🚀 🥊


    for (let i = 0; i < ALL_UPDATER.length; i++) {
        ALL_UPDATER[i].update()
    }
}