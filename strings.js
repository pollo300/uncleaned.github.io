...
AI(){
    ...
    if(this.target.rank>this.owner.rank) {
        ...
        far = Math.min(Math.max(this.fov/(dist+this.target.r+this.owner.r),0),1)*doge
    }
    if(this.target.dmghp>0) {
        // gradual ram if you have more healt, but avoid if you healt percentage is low
        run = Math.min(Math.max((-this.owner.hp*this.owner.dmg*(this.owner.hp/this.owner.maxhp) + this.target.dmghp),0),1)
    } else {
        // not ram death
        run = 1
    }
    ...
    if(this.obsta.dmghp>0 && obst_dist*2.5<dist+this.target.r+this.owner.r){
...
{
    this.disance1 = this.owner.r + this.owner.r * (this.bl_st.rel_vel/30) //* ((Main/2-Math.abs(i2))/2+1)
